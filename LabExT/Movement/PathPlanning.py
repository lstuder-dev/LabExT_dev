...
class CollisionAvoidancePlanning(PathPlanning):
    ...
    def _get_grid_properties(
        ...
        print(np.floor(np.min(pdist(all_points))))
        print(grid_size)
        if grid_size <= 10: # hacky fix
            #grid_size = np.unique(np.floor(pdist(all_points)))[1]

            print("Warning: the grid size was manually set to 100 um, so solve runtime errors!!!")
            grid_size = 100
